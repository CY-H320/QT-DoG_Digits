...
class Digits(MultipleDomainDataset):
    """A dataset combining multiple digit recognition datasets:
    - MNIST (torchvision)
    - SynDigits (custom folder)
    - MNIST-M (custom folder)
    """

    ENVIRONMENTS = ["MNIST", "SynDigits", "MNIST-M"]
    INPUT_SHAPE = (3, 32, 32)
    NUM_CLASSES = 10
    ...
    def __init__(self, root):
        ...
        self.datasets = [
            self.load_mnist(root),
            self.load_syn_digits(os.path.join(root, "SynDigits")),
            self.load_mnist_m(os.path.join(root, "MNIST-M"))
        ]
        self.environments = self.ENVIRONMENTS
        self.input_shape = self.INPUT_SHAPE
        self.num_classes = self.NUM_CLASSES

    def load_mnist(self, root):
        transform = T.Compose([
        ...
        return mnist_train  # Return only the training dataset


    def load_syn_digits(self, root):
        return self.load_image_folder(root)

    def load_mnist_m(self, root):
        return self.load_image_folder(root)

    def load_image_folder(self, root):
        """Loads SynDigits or MNIST-M from a directory structure."""
        transform = T.Compose([
            T.Resize((32, 32)),
            T.ToTensor()
        ])
        train_dataset = ImageFolder(os.path.join(root, "train"), transform=transform)
        test_dataset = ImageFolder(os.path.join(root, "test"), transform=transform)
